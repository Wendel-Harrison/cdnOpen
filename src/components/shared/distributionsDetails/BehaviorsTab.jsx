...
import { Route, Server, ArrowUp, ArrowDown, Edit, Trash2 } from 'lucide-react';
  ...
  if (!behaviors || behaviors.length === 0) {
    return (
      <div className="text-center py-16 bg-white dark:bg-card rounded-2xl shadow border dark:border-border">
        ...
        <p className="text-muted-foreground text-sm">Nenhum behavior (rota) cadastrado nesta distribuição.</p>
      </div>
    );
  }
            ...
            {behaviors.map((behavior, index) => (
                <tr key={behavior.id} className="border-b last:border-0 border-slate-100 dark:border-border/50 hover:bg-slate-50 dark:hover:bg-neutral-950/30 transition-colors duration-300">
                  ...
                  <td className="px-6 py-2">
                    <span className="font-mono bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded text-xs text-foreground font-bold">{behavior.path_pattern}</span>
                  </td>
                  <td className="px-6 py-2">
